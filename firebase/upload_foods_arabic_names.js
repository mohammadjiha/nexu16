...
async function commitBatch(accessToken, writes) {
  ...
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    console.log(`Commit hit ${response.status}; retrying in ${Math.round(delayMs / 1000)}s...`);
    await new Promise((resolve) => setTimeout(resolve, delayMs));
  }
  throw new Error("Commit failed after retries.");
}

function pickUploadFields(food) {
  return {
    externalId: String(food.externalId || food.id || ""),
    importIndex: Number(food.importIndex) || 0,
    name: String(food.name || ""),
    nameLower: String(food.nameLower || food.name || "").toLowerCase(),
    namePrefixes: Array.isArray(food.namePrefixes) ? food.namePrefixes : [],
    nameAr: String(food.nameAr || ""),
    namePrefixesAr: Array.isArray(food.namePrefixesAr) ? food.namePrefixesAr : [],
    emoji: String(food.emoji || "🍽️"),
    servingSize: String(food.servingSize || "100g"),
    ...
    tags: Array.isArray(food.tags) ? food.tags.map(String) : [],
    tagText: Array.isArray(food.tags) ? food.tags.join(" ").toLowerCase() : "",
    source: "20k_gym_foods_ar_deduped.json",
    arabicUpdatedAt: new Date().toISOString(),
  };
}
...
async function main() {
  ...
  for (let i = startIndex; i < foods.length; i += 1) {
    const food = foods[i];
    const docId = String(food.docId || "").trim();
    if (!docId) throw new Error(`Missing docId at index ${i}`);
    const normalized = pickUploadFields(food);
    writes.push({
      update: {
        name: documentName(docId),
        fields: Object.fromEntries(Object.entries(normalized).map(([key, value]) => [key, firestoreValue(value)])),
      },
    });
  ...
  console.log(`Done. Uploaded ${uploaded} Arabic food documents to ${collection}. Firestore count: ${total ?? "unknown"}.`);
}
